...
type CareerContext = {
  career_deep_dive: string | null
  guidelines_tips: string | null
}

interface StoryFormProps {
  existingArtifacts: ExistingArtifact[]
  existingContext: CareerContext | null
}

export function StoryForm({ existingArtifacts, existingContext }: StoryFormProps) {
  const router = useRouter()

  // Split existing artifacts into CV and supporting
  const existingCv = existingArtifacts.find(a => a.is_cv)
  const existingSupportingDocs = existingArtifacts.filter(a => !a.is_cv && a.file_name !== 'career-context.txt')

  const [cvFile, setCvFile] = useState<File | null>(null)
  const [hasExistingCv, setHasExistingCv] = useState(!!existingCv)
  const [supportingFiles, setSupportingFiles] = useState<UploadedFile[]>([])
  const [existingSupporting, setExistingSupporting] = useState<ExistingArtifact[]>(existingSupportingDocs)
  const [careerContext, setCareerContext] = useState(existingContext?.career_deep_dive || '')
  const [guidelinesTips, setGuidelinesTips] = useState(existingContext?.guidelines_tips || '')
  ...
  const handleCvDrop = useCallback((e: React.DragEvent) => {
    ...
    if (file) {
      ...
      setCvFile(file)
      setHasExistingCv(false) // Will replace existing
      setError(null)
    }
  }, [])
  ...
  const removeExistingSupporting = async (artifactId: string) => {
    try {
      const response = await fetch(`/api/artifacts/${artifactId}`, {
        method: 'DELETE',
      })
      if (response.ok) {
        setExistingSupporting(prev => prev.filter(a => a.id !== artifactId))
  ...
  const saveCareerContext = async () => {
    setIsSaving(true)
    try {
      const response = await fetch('/api/career-context', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          career_deep_dive: careerContext,
          guidelines_tips: guidelinesTips,
        }),
      })
  ...
  // Auto-save career context on blur
  const handleContextBlur = () => {
    if (careerContext !== (existingContext?.career_deep_dive || '') ||
        guidelinesTips !== (existingContext?.guidelines_tips || '')) {
      saveCareerContext()
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (cvFile) {
        ...
        uploadedArtifactIds.push(existingCv.id)
      }
      ...
      // Include existing supporting docs
      existingSupporting.forEach(a => uploadedArtifactIds.push(a.id))

      // Save career context to dedicated table
      await saveCareerContext()

      toast.success(`Processing ${uploadedArtifactIds.length} file(s)`)
  ...
  const hasNewContent = cvFile || supportingFiles.length > 0
    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <CardContent>
          <div
            ...
            className={`
              ...
              ${cvFile || hasExistingCv ? 'border-green-500 bg-green-50' : 'border-gray-300 hover:border-gray-400'}
            `}
          >
            {cvFile ? (
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-blue-600">New file - will replace existing</p>
                </div>
                <Button variant="ghost" size="sm" onClick={() => { setCvFile(null); setHasExistingCv(!!existingCv) }}>
                  Remove
                </Button>
              </div>
            ) : hasExistingCv && existingCv ? (
              <div className="flex items-center justify-center gap-3">
                <svg className="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <div className="text-left">
                  <p className="font-medium text-gray-900">{existingCv.file_name}</p>
        ...
        <CardContent>
          ...
          {existingSupporting.length > 0 && (
            <div className="mt-4 space-y-2">
              ...
              {existingSupporting.map((artifact) => (
                <div key={artifact.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{artifact.file_name}</p>
                      <p className="text-xs text-gray-500">Uploaded previously</p>
                    </div>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => removeExistingSupporting(artifact.id)}>
          ...
          {supportingFiles.length > 0 && (
            <div className="mt-4 space-y-2">
              {existingSupporting.length > 0 && <p className="text-xs text-gray-500 font-medium">New files:</p>}
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-gray-500 mt-2">
            This helps us extract richer insights from your documents. Auto-saves when you click away.
          </p>
        </CardContent>
      </Card>

      {/* Guidelines & CV Tips - New Section */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            Guidelines & CV Writing Preferences
            <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">Optional</span>
          </CardTitle>
          <CardDescription>
            Personal CV rules, style preferences, or tips from a career coach
          </CardDescription>
        </CardHeader>
        <CardContent>
          <textarea
            value={guidelinesTips}
            onChange={(e) => setGuidelinesTips(e.target.value)}
            onBlur={handleContextBlur}
            placeholder="E.g., 'Always lead with impact metrics', 'Keep bullet points under 2 lines', 'Avoid buzzwords like synergy', 'Use action verbs'..."
            className="w-full h-32 p-4 border border-gray-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
          />
          <p className="text-xs text-gray-500 mt-2">
            These preferences will be applied when generating your tailored resumes. Auto-saves when you click away.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="flex gap-4">
        ...
        <Button
          ...
          disabled={(!cvFile && !hasExistingCv) || isUploading}
          className="flex-1"
        >
          {isUploading ? (
            <>
